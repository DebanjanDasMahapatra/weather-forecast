...
const BaseInput = () => {
    ...
    const getWeatherAndtGeoLocation = () => {
        ...
        axios.all([
            ...
            axios.get(Constants.GeoLocationURL.replace("LAT", latitude.toString()).replace("LNG", longitude.toString()))
        ])
    ...
    const selectSearchResults = (id: number) => {
        ...
        if (locationResult) {
            ...
            setGeoLocation(getCompleteLocation(locationResult))
            setState("searching")
            setSearchText("")
        }
    }
    ...
    return <>
        ...
        <Container>
            ...
            {locationType == "other" && <Form.Group controlId="type-field">
                <Form.Control type="text" placeholder="Search Here..." value={searchText} onChange={$e => setSearchText($e.target.value)} />
            </Form.Group>}
            {searchResults.length != 0 && <ListGroup>
                {searchResults.map((searchResult: GeoCodingSearchResults) => <ListGroup.Item className="bg-dark text-light" style={{ borderColor: 'grey' }} key={searchResult.id} action onClick={() => selectSearchResults(searchResult.id)}>{getCompleteLocation(searchResult)}</ListGroup.Item>)}
            </ListGroup>}
        </Container>
        <br />
        ...
        <br />
        {state == "found" && <WeatherForecastTable weatherData={weatherData} airQualityData={airQualityData} />}
        <hr />
    </>
}
